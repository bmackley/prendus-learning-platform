...
const resolveVideoIds = async (videoIds: string[]): Promise<Video[]> => {
    try {
      const videos = await asyncReduce(videoIds, []);
      return videos;

      async function asyncReduce(videoIds: string[], videos: Video[]): Promise<Video[]> {
          ...
          return asyncReduce(videoIds.slice(1), [...videos, video]);
      }
    }
    catch(error) {
      throw error;
