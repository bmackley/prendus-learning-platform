...
const resolveQuizIds = async (quizIds: string[]): Promise<Quiz[]> => {
    try {
      const quizzes = await asyncReduce(quizIds, []);
      return quizzes;

      async function asyncReduce(quizIds: string[], quizzes: Quiz[]): Promise<Quiz[]> {
          ...
          return asyncReduce(quizIds.slice(1), [...quizzes, quiz]);
      }
    }
    catch(error) {
      throw error;
