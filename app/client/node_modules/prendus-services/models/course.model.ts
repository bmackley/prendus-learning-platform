...
const getConceptIds = async (courseId: string): Promise<string[]> => {
    try {
        const path = `${dataPath}/${courseId}/concepts`;
        const conceptIdsObject = await FirebaseService.get(path);
        const conceptIds = Object.keys(conceptIdsObject || {});
